...
} from '../../accounting/hooks/useInvoiceTemplates';
...
const INVOICE_TYPES = [
  ...
  { value: 'FRAIS_DEMARCHES_ACD', label: 'Frais de démarches ACD' },
...
export default function InvoiceTemplatesSettingsTab() {
  const { data: res, isLoading } = useInvoiceTemplates();

  if (isLoading) return <SkeletonTable rows={6} />;
  const d = res?.data;
  if (!d) return <div className="text-slate-500">Modèles indisponibles.</div>;
