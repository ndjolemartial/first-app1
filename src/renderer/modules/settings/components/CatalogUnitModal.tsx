...
import { useCatalogUnits, useSaveCatalogUnit, useDeleteCatalogUnit } from '../../../shared/hooks/useCatalog';

interface Props {
  open: boolean;
  onClose: () => void;
  /** Notifié après création d'une unité (pour la pré-sélectionner sur l'article). */
  onCreated?: (label: string) => void;
}

/**
 * Fenêtre d'édition des unités de mesure (référentiel partagé `KpiUnit`, le
 * même que « Nouvel objectif » et les lignes de devis). Permet d'ajouter,
 * renommer et supprimer les valeurs proposées dans le champ « Unité » du
 * catalogue prestations/produits.
 */
export default function CatalogUnitModal({ open, onClose, onCreated }: Props) {
  const { data: res } = useCatalogUnits();
  const list: any[] = res?.success ? res.data ?? [] : [];

  const save = useSaveCatalogUnit();
  const remove = useDeleteCatalogUnit();
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message={`Supprimer « ${toDelete?.label} » ? Les articles utilisant déjà cette unité ne seront pas modifiés.`}
