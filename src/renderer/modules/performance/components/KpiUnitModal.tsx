...
import { useKpiUnits, useSaveKpiUnit, useDeleteKpiUnit } from '../hooks/usePerformance';

interface Props {
  open: boolean;
  onClose: () => void;
  /** Notifié après création d'une unité (pour la pré-sélectionner dans le formulaire). */
  onCreated?: (label: string) => void;
}

/**
 * Fenêtre d'édition des unités de KPI. Permet d'ajouter, renommer et supprimer
 * les valeurs proposées dans le champ « Unité » du formulaire KPI — même principe
 * que les postes de la fiche employé.
 */
export default function KpiUnitModal({ open, onClose, onCreated }: Props) {
  const { data: res } = useKpiUnits();
  const list: any[] = res?.success ? res.data ?? [] : [];

  const save = useSaveKpiUnit();
  const remove = useDeleteKpiUnit();
  ...
  return (
    <>
        ...
        <div className="space-y-5">
          <div className="rounded-lg border border-slate-200 bg-slate-50/60 p-4 space-y-3">
            ...
            <Input
              ...
              placeholder="Ex : FCFA, %, nb, h, jours…"
            />
      ...
      <ConfirmDialog
        ...
        message={`Supprimer « ${toDelete?.label} » ? Les KPI utilisant déjà cette unité ne seront pas modifiés.`}
