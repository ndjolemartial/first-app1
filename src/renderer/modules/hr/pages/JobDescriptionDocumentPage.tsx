...
import { useContractRenderData, useMyContractRenderData, useJobDescriptionTemplates } from '../hooks/useHr';
...
/**
 * Fiche de poste d'un contrat : rendue à la demande depuis les données du
 * contrat (employé, fonction, autorité…) et un modèle éditable de fiche de poste.
 */
export default function JobDescriptionDocumentPage({ selfMode = false }: { selfMode?: boolean }) {
  ...
  const { data: templatesRes } = useJobDescriptionTemplates();
  ...
  // Modèles de fiche de poste — le modèle par défaut en premier.
  ...
  const exportFileName = `Fiche-de-poste-${contract.reference}${sanitized ? '-' + sanitized : ''}`;
  ...
  return (
    <PageLayout
      title={`Fiche de poste — ${contract.reference}`}
      breadcrumbs={selfMode
        ? [{ label: 'Mon espace RH', to: '/my-hr' }, { label: 'Fiche de poste' }]
        : [
            { label: 'RH & Paie' },
      ...
      {templates.length === 0 ? (
        <EmptyState
          ...
          description={selfMode
            ? "Aucun modèle de fiche de poste n'est disponible. Contactez le service RH."
            : 'Créez un modèle de fiche de poste dans les Paramètres pour générer le document.'}
          action={selfMode ? undefined : { label: 'Créer un modèle', onClick: () => navigate('/hr/job-descriptions/templates/new') }}
        />
      ) : (
        <div className="space-y-4">
          <Card className="flex flex-wrap items-end gap-3">
            ...
            <p className="text-xs text-slate-500 pb-2">
              Les variables dynamiques sont remplacées par les données du contrat, de l'employé et de la fonction.
            </p>
          </Card>
