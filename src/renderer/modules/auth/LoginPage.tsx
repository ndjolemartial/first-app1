...
export default function LoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setError(null);
    try {
      const res = await window.electron.auth.login(data.email, data.password);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1E3A5F] to-[#2563EB] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          <div className="h-14 w-14 rounded-2xl bg-blue-600 flex items-center justify-center text-white font-bold text-2xl mb-4">A</div>
          <h1 className="text-2xl font-bold text-slate-900">Afrikimmo-App</h1>
