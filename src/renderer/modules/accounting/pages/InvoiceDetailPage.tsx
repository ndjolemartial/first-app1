...
export default function InvoiceDetailPage() {
  ...
  // Date du jour (YYYY-MM-DD) pré-remplie comme date de paiement par défaut.
  const today = new Date().toISOString().slice(0, 10);
  const { register, handleSubmit, reset, watch, setValue, formState: { isSubmitting } } = useForm({
    defaultValues: { amount: '', method: 'ESPECE', paidAt: today, reference: '', notes: '' },
  });
  ...
  const onPaymentSubmit = async (data: any) => {
    const r = await addPayment.mutateAsync({
      ...
      payload: {
        ...
        method: data.method,
        // Date de paiement saisie (YYYY-MM-DD) convertie en ISO ; sinon le
        // back-end applique la date du jour.
        paidAt: data.paidAt ? new Date(`${data.paidAt}T00:00:00.000Z`).toISOString() : undefined,
