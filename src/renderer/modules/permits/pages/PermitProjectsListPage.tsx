...
import { usePermitProjects, useDeletePermitProject } from '../hooks/usePermitProjects';
import { NATURE_LABELS, STANDING_LABELS, PROJECT_STATUS_LABELS, PROJECT_STATUS_VARIANT, toOptions, formatFcfa } from '../utils/permitLabels';
...
export default function PermitProjectsListPage() {
  const navigate = useNavigate();
  const [status, setStatus] = useState('');
  const [search, setSearch] = useState('');

  const filters = { ...(status ? { status } : {}), ...(search ? { search } : {}) };
  const { data: res, isLoading } = usePermitProjects(filters, 1, 100);
  ...
  return (
    <PageLayout
      title="Devis permis de construire"
      breadcrumbs={[{ label: 'Devis permis de construire' }]}
      actions={canWrite ? (
        <Button icon={<Plus className="h-4 w-4" />} onClick={() => navigate('/permits/projects/new')}>
          Nouveau projet
        </Button>
      ) : undefined}
    >
      ...
      <Card className="!p-0 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-slate-400">
            ...
            Aucun projet de permis de construire.
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-slate-600">Nature</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600">Standing</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600">Coût des travaux</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {projects.map((p: any) => (
                <tr key={p.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => navigate(`/permits/projects/${p.id}`)}>
                  ...
                  <td className="px-4 py-3 text-slate-500">{NATURE_LABELS[p.nature] ?? p.nature}</td>
                  <td className="px-4 py-3 text-slate-500">{STANDING_LABELS[p.standing] ?? p.standing}</td>
                  <td className="px-4 py-3 text-right text-slate-700">{formatFcfa(p.coutPrevisionnelTravaux != null ? Number(p.coutPrevisionnelTravaux) : null)}</td>
