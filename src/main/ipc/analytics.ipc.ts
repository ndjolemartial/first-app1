...
import { computeBalances } from '../services/treasury.service';
...
function monthBuckets(now: Date, count: number): Array<{ label: string; start: Date; end: Date }> {
...
async function computeEncaisse(db: Db, start: Date, end: Date): Promise<number> {
...
async function computeDecaisse(db: Db, start: Date, end: Date): Promise<number> {
 ...
 */
async function computeEntrees(db: Db, start: Date, end: Date): Promise<number> {
  const agg = await db.treasuryOperation.aggregate({
    where: {
      direction: 'ENTREE',
      deletedAt: null,
...
async function computeUnpaidInvoices(db: Db): Promise<{ amount: number; count: number }> {
...
async function outstandingInstallments(db: Db): Promise<{ amount: number; count: number; overdueAmount: number; overdueCount: number }> {
  ...
  for (const i of list) {
    ...
    amount += due;
    if (i.status === 'EN_RETARD') { overdueAmount += due; overdueCount += 1; }
  }
...
function guardManagerFullAccess(token: string) {
  const session = getSession(token);
  if (!session) throw new Error('Session expirée');
  if (!MANAGER_FULL_ACCESS_ROLES.includes(session.role)) throw new Error('Permission insuffisante');
  return session;
}
...
function guardFollowUpAccess(token: string) {
  const session = getSession(token);
  if (!session) throw new Error('Session expirée');
  if (!FOLLOWUP_ROLES.includes(session.role)) throw new Error('Permission insuffisante');
  return session;
}

const DAY_MS = 86400000;

/** État de suivi calculé à partir du nombre de jours d'inaction. */
type FollowUpState = 'NORMAL' | 'NEGLIGE' | 'DANGER' | 'CRITIQUE';
 ...
 */
function classifyFollowUp(daysSince: number, [t1, t2, t3]: [number, number, number]): FollowUpState {
  if (daysSince < t1) return 'NORMAL';
  if (daysSince < t2) return 'NEGLIGE';
  if (daysSince < t3) return 'DANGER';
  return 'CRITIQUE';
}
...
const PROSPECT_THRESHOLDS: [number, number, number] = [14, 60, 90];
...
const CLIENT_THRESHOLDS: [number, number, number] = [90, 180, 270];

export function registerAnalyticsIPC(): void {
  /* ─── 1. Tableau de bord exécutif ──────────────────────────────────── */
  ipcMain.handle('analytics:executive', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      const caEvolution = [];
      for (const b of buckets) caEvolution.push({ label: b.label, encaisse: await computeEncaisse(db, b.start, b.end) });

      const [caYear, caMonth, unpaid, outstanding, activeConv, clientsCount,
        terrainsDispo, propsDispo, commAgg, accounts, forecastDue] = await Promise.all([
      ...
      logger.error('analytics:executive error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 2. Analyse financière & rentabilité ──────────────────────────── */
  ipcMain.handle('analytics:financial', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      for (const b of buckets) {
        ...
        totalRecettes += recettes; totalDepenses += depenses;
      ...
      logger.error('analytics:financial error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 3. Analyse du portefeuille immobilier ────────────────────────── */
  ipcMain.handle('analytics:portfolio', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const [propsByStatus, propsByType, terrainsByStatus,
        terrainsDispoVal, propsDispoVal, terrainsVenduVal, propsVenduVal,
        lotissements, programmes] = await Promise.all([
      ...
      logger.error('analytics:portfolio error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 4. Analyse CRM, souscripteurs & locataires ───────────────────── */
  ipcMain.handle('analytics:crm', async (_event, { token }: any) => {
    try {
      ...
      const souscripteursWhere: any = { deletedAt: null, status: 'ACTIVE', type: { in: ['SALE', 'SOUSCRIPTION'] } };
      const [prospectsByStatus, prospectsBySource, prospectsTotal, converted,
        clientsByType, locataires, souscripteursTerrains, souscripteursProperties, actPending, actOverdue] = await Promise.all([
      ...
      logger.error('analytics:crm error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 4bis. Détail (liste) d'un indicateur CRM cliqué ──────────────── */
  ipcMain.handle('analytics:crmDetail', async (_event, { token, metric, source, page = 1, limit = 10 }: any) => {
    try {
      guardManagerFullAccess(token);
      const db = getDb();
      const now = new Date();
      ...
      switch (metric) {
        ...
        case 'converted': {
          const where = { deletedAt: null, status: 'CONVERTI' as const };
          const [data, total] = await db.$transaction([
        ...
        case 'locataires': {
          const where: any = { deletedAt: null, status: 'ACTIVE', type: { in: ['RENTAL_UNFURNISHED', 'RENTAL_FURNISHED', 'COMMERCIAL_LEASE'] } };
          const [data, total] = await db.$transaction([
        ...
        case 'souscripteurs': {
          const where: any = { deletedAt: null, status: 'ACTIVE', type: { in: ['SALE', 'SOUSCRIPTION'] } };
          const [data, total] = await db.$transaction([
            db.convention.findMany({ where, select: CONVENTION_SELECT, skip, take: limit, orderBy: { startDate: 'desc' } }),
            db.convention.count({ where }),
          ]);
        ...
        case 'activitiesPending': {
          const where: any = { status: { in: ['EN_ATTENTE', 'EN_TRAITEMENT'] } };
          const [data, total] = await db.$transaction([
        ...
        case 'activitiesOverdue': {
          const where: any = { status: { in: ['EN_ATTENTE', 'EN_TRAITEMENT'] }, dueDate: { lt: now } };
          const [data, total] = await db.$transaction([
            db.crmActivity.findMany({ where, select: ACTIVITY_SELECT, skip, take: limit, orderBy: { dueDate: 'asc' } }),
            db.crmActivity.count({ where }),
          ]);
      ...
      logger.error('analytics:crmDetail error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 5. Analyse des charges ───────────────────────────────────────── */
  ipcMain.handle('analytics:charges', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      for (const b of buckets) depensesEvolution.push({ label: b.label, depenses: await computeDecaisse(db, b.start, b.end) });
      ...
      logger.error('analytics:charges error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 6. Analyse contractuelle ─────────────────────────────────────── */
  ipcMain.handle('analytics:contracts', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      logger.error('analytics:contracts error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 7. Gestion des risques ───────────────────────────────────────── */
  ipcMain.handle('analytics:risk', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      logger.error('analytics:risk error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 8. Aide à la décision & recommandations ──────────────────────── */
  ipcMain.handle('analytics:recommendations', async (_event, { token }: any) => {
    try {
      guard(token);
      const db = getDb();
      const now = new Date();
      ...
      const [outstanding, unpaid, collectedYear, invoicedAgg, expiring, forecastOverdue,
        stockTerrains, stockProps, commToPay, accounts] = await Promise.all([
        outstandingInstallments(db),
        computeUnpaidInvoices(db),
        computeEncaisse(db, yearStart, now),
        db.invoice.aggregate({ where: { deletedAt: null, status: { not: 'ANNULEE' }, issueDate: { gte: yearStart } }, _sum: { total: true } }),
      ...
      const recos: Array<{ severity: 'high' | 'medium' | 'low'; domain: string; title: string; detail: string }> = [];
      ...
      logger.error('analytics:recommendations error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 9. Suivi prospects & clients (fréquence & plan de relance) ───── */
  ipcMain.handle('analytics:followUp', async (_event, { token }: any) => {
    try {
      ...
      const prospectWhere: any = { deletedAt: null, status: { not: 'PERDU' } };
      if (!isFullAccess) prospectWhere.assignedToId = session.userId;
      ...
      const clientWhere: any = { deletedAt: null, status: { not: 'SUSPENDU' } };
      if (!isFullAccess) clientWhere.assignedToId = session.userId;
      ...
      const referrerWhere: any = { deletedAt: null, isActive: true };
      if (!isFullAccess) referrerWhere.assignedToId = session.userId;
      ...
      const frequency = [];
      for (const b of buckets) {
        const prospectActWhere: any = { prospectId: { not: null }, createdAt: { gte: b.start, lt: b.end } };
        const clientActWhere: any = { clientId: { not: null }, createdAt: { gte: b.start, lt: b.end } };
        const referrerCommissionWhere: any = { referrerId: { not: null }, deletedAt: null, createdAt: { gte: b.start, lt: b.end } };
        const referrerDocumentWhere: any = { referrerId: { not: null }, deletedAt: null, uploadedAt: { gte: b.start, lt: b.end } };
        const referrerTimelineWhere: any = { entityType: 'REFERRER', createdAt: { gte: b.start, lt: b.end } };
        if (!isFullAccess) {
      ...
      const prospectLastActionMap = new Map<number, Date>();
      for (const g of prospectLastActions) if (g.prospectId != null && g._max.createdAt) prospectLastActionMap.set(g.prospectId, g._max.createdAt);
      const clientLastActionMap = new Map<number, Date>();
      for (const g of clientLastActions) if (g.clientId != null && g._max.createdAt) clientLastActionMap.set(g.clientId, g._max.createdAt);
      ...
      const referrerLastActionMap = new Map<number, Date>();
      const bumpLastAction = (id: number | null | undefined, date: Date | null | undefined) => {
        if (id == null || !date) return;
        const current = referrerLastActionMap.get(id);
        if (!current || date > current) referrerLastActionMap.set(id, date);
      };
      for (const g of referrerCommissionLast) bumpLastAction(g.referrerId, g._max.createdAt);
      for (const g of referrerDocumentLast) bumpLastAction(g.referrerId, g._max.uploadedAt);
      for (const g of referrerTimelineLast) bumpLastAction(g.entityId, g._max.createdAt);
      ...
      const STATES: FollowUpState[] = ['NORMAL', 'NEGLIGE', 'DANGER', 'CRITIQUE'];
      const countBy = (items: Array<{ state: FollowUpState }>) =>
        STATES.map((state) => ({ key: state, count: items.filter((i) => i.state === state).length }));
      ...
      logger.error('analytics:followUp error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 10. Statistiques visiteurs ────────────────────────────────────── */
  ipcMain.handle('analytics:visitors', async (_event, { token }: any) => {
    try {
      guardManagerFullAccess(token);
      const db = getDb();
      const now = new Date();
      ...
      logger.error('analytics:visitors error', error.message);
      return { success: false, error: error.message };
    }
  });

  /* ─── 11. Statistiques appels ───────────────────────────────────────── */
  ipcMain.handle('analytics:calls', async (_event, { token }: any) => {
    try {
      guardManagerFullAccess(token);
      const db = getDb();
      const now = new Date();
      ...
      logger.error('analytics:calls error', error.message);
      return { success: false, error: error.message };
    }
  });
}
