 ...
 */
function extractCoords(text: string): { latitude: number; longitude: number } | null {
...
function extractAll(text: string): { latitude: number; longitude: number } | null {
...
export function registerGeoIPC(): void {
   ...
   */
  ipcMain.handle('geo:resolveMapLink', async (_event, { token, link }: any) => {
    try {
      const session = getSession(token);
      if (!session) return { success: false, error: 'Session expirée' };

      const raw = String(link ?? '').trim();
      if (!raw) return { success: false, error: 'Lien vide' };
      ...
      logger.info(`geo:resolveMapLink résolu → ${coords.latitude}, ${coords.longitude}`);
      return { success: true, data: coords };
    } catch (error: any) {
      const reason = error?.name === 'AbortError' ? 'délai dépassé' : error?.message ?? 'erreur inconnue';
      logger.error('geo:resolveMapLink error', reason);
