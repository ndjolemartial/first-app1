...
const conventionBaseSchema = z.object({
  ...
  priorConventionDate: z.string().datetime().optional(),
  ...
  priorTotalVersements: z.number().optional(),
  priorSolde: z.number().optional(),
  priorTotalBiens: z.number().optional(),
  ...
  lotsSouscrits: z.string().optional(),
  ...
  lotsAnterieursSouscrits: z.string().optional(),
 ...
 * + échéancier).
 */
function isPaymentOptionType(type?: string, amendmentType?: string | null): boolean {
  if (!type || !PAYMENT_OPTION_TYPES.includes(type)) return false;
 ...
 */
function usesPriorTerrains(type?: string, amendmentType?: string | null): boolean {
...
export function registerConventionsIPC(): void {
  ...
  ipcMain.handle('conventions:create', async (_event, { token, payload }: any) => {
    try {
      ...
      if (priorTerrainIds.length > 0) await assertSingleLotissement(db, priorTerrainIds);
      ...
      if (isPaymentOptionType(d.type, d.amendmentType)) {
        ...
        if (additionnel > 0) autoInvoices.push({ type: 'OTHER', amount: additionnel, label: 'Paiement additionnel' });
      } else {
        const fraisOuverture = Number(d.fraisOuvertureDossier ?? 0);
        const apport = Number(d.apportInitial ?? 0);
        if (fraisOuverture > 0) autoInvoices.push({ type: 'FRAIS_OUVERTURE_DOSSIER', amount: fraisOuverture, label: "Frais d'ouverture de dossier" });
        if (apport > 0) autoInvoices.push({ type: 'APPORT_INITIAL', amount: apport, label: 'Apport initial' });
      ...
      const autoInvoiceStatus: 'VALIDEE' | 'BROUILLON' | null =
        convention.status === 'ACTIVE' ? 'VALIDEE'
      ...
      if (autoInvoices.length > 0 && autoInvoiceStatus) {
        ...
        for (const inv of autoInvoices) {
          ...
          await db.invoice.create({
            data: {
              ...
              clientId: d.clientId,
              conventionId: convention.id,
              subtotal: inv.amount as any,
              taxRate: 0 as any,
              taxAmount: 0 as any,
              total: inv.amount as any,
              issueDate: now,
              dueDate: now,
              items: {
                create: [{
        ...
        logger.info(`Convention ${convention.reference}: ${autoInvoices.length} facture(s) ${autoInvoiceStatus} générée(s) automatiquement`);
  ...
  ipcMain.handle('conventions:update', async (_event, { token, id, payload }: any) => {
    try {
      ...
      for (const k of Object.keys(parsed.data)) {
        if (sentKeys.has(k)) d[k] = (parsed.data as any)[k];
      }
      const data: any = { ...d };
      ...
      if (d.priorConventionDate) data.priorConventionDate = new Date(d.priorConventionDate);
      ...
      if ('lotsSouscrits' in data) data.lotsSouscrits = data.lotsSouscrits || null;
      // Lots antérieurs souscrits : avenant de transfert de site uniquement.
      ...
      if (!effWithPrior) data.lotsAnterieursSouscrits = null;
      if ('lotsAnterieursSouscrits' in data) data.lotsAnterieursSouscrits = data.lotsAnterieursSouscrits || null;
      ...
      if (d.type && !INHERITED_CONVENTION_TYPES.includes(d.type)) data.priorConventionDate = null;
      // Les conditions particulières ne concernent que les conventions héritées.
      if (d.type && !INHERITED_CONVENTION_TYPES.includes(d.type)) data.conditionsParticulieres = null;
      if ('conditionsParticulieres' in data) data.conditionsParticulieres = data.conditionsParticulieres || null;
      ...
      if (d.propertyIds !== undefined || d.assetType === 'TERRAIN') {
        ...
        if (effectiveAssetType === 'PROPERTY' && d.propertyIds && d.propertyIds.length > 0) {
          await db.conventionProperty.createMany({
            data: d.propertyIds.map((propertyId: number, i: number) => ({ conventionId: id, propertyId, order: i })),
          });
        }
      }
      if (d.terrainIds !== undefined || d.assetType === 'PROPERTY') {
        ...
        if (effectiveAssetType === 'TERRAIN' && d.terrainIds && d.terrainIds.length > 0) {
          await db.conventionTerrain.createMany({
            data: d.terrainIds.map((terrainId: number, i: number) => ({ conventionId: id, terrainId, order: i })),
          });
        }
      }
      ...
      if (d.priorTerrainIds !== undefined || !effWithPrior || d.assetType === 'PROPERTY') {
        if (d.priorTerrainIds && d.priorTerrainIds.length > 0) {
          ...
          const currentTerrainIds = d.terrainIds ?? (
            await db.conventionTerrain.findMany({ where: { conventionId: id }, select: { terrainId: true } })
          ).map((r) => r.terrainId);
          if (d.priorTerrainIds.some((tid: number) => currentTerrainIds.includes(tid))) {
            return { success: false, error: 'Un même terrain ne peut pas figurer à la fois dans « Terrains rattachés » et « Terrains antérieurs rattachés ».' };
          }
        }
        ...
        if (effWithPrior && effectiveAssetType === 'TERRAIN' && d.priorTerrainIds && d.priorTerrainIds.length > 0) {
          ...
          await db.conventionPriorTerrain.createMany({
            data: d.priorTerrainIds.map((terrainId: number, i: number) => ({ conventionId: id, terrainId, order: i })),
          });
        }
      }
      ...
      if (d.installments) {
        ...
        if (d.installments.length > 0) {
          await db.saleInstallment.createMany({
            data: d.installments.map((inst: { dueDate: string; amount: number }, i: number) => ({
              conventionId: id,
              installmentNumber: i + 1,
              dueDate: new Date(inst.dueDate),
              amount: inst.amount as any,
  ...
  ipcMain.handle('conventions:delete', async (_event, { token, id }: any) => {
    try {
      ...
      await db.$transaction(async (tx) => {
        ...
        const treasuryOr: any[] = [];
        if (paymentIds.length) treasuryOr.push({ paymentId: { in: paymentIds } });
        if (installmentIds.length) treasuryOr.push({ installmentId: { in: installmentIds } });
        if (commissionIds.length) treasuryOr.push({ commissionId: { in: commissionIds } });
        if (treasuryOr.length) await tx.treasuryOperation.deleteMany({ where: { OR: treasuryOr } });
        ...
        if (invoiceIds.length) await tx.document.updateMany({ where: { invoiceId: { in: invoiceIds } }, data: { invoiceId: null } });
        if (commissionIds.length) await tx.document.updateMany({ where: { commissionId: { in: commissionIds } }, data: { commissionId: null } });
        await tx.crmActivity.updateMany({ where: { conventionId: id }, data: { conventionId: null } });
        if (invoiceIds.length) await tx.crmActivity.updateMany({ where: { invoiceId: { in: invoiceIds } }, data: { invoiceId: null } });
        if (installmentIds.length) await tx.crmActivity.updateMany({ where: { installmentId: { in: installmentIds } }, data: { installmentId: null } });
        ...
        if (commissionIds.length) await tx.commission.deleteMany({ where: { id: { in: commissionIds } } });

        // 4) Suppression définitive de la convention (cascade sur les liens terrains/biens)
        await tx.convention.delete({ where: { id } });
      });
      logger.info(`Convention #${id} supprimée définitivement (factures, échéances, commission, trésorerie liées)`);
      return { success: true };
    } catch (error: any) {
      logger.error('conventions:delete', error.message);
      return { success: false, error: error.message };
    }
  });
