...
function checkBilanRole(session: { role: string }): void {
  if (session.role === 'ASSISTANTE_DIRECTION') throw new Error('Permission insuffisante');
  checkRole(session as any, READ_ROLES);
}

function checkActifPassifRole(session: { role: string }): void {
  checkRole(session as any, ACTIF_PASSIF_ROLES);
}

// ── Helpers période ──────────────────────────────────────────────────────────

type PeriodType = 'month' | 'last-month' | 'quarter' | 'semester' | 'year' | 'custom';
...
function computePeriod(
  ...
  if (periodType === 'custom') {
    if (!periodStart || !periodEnd) throw new Error('Période personnalisée incomplète');
 ...
 */
function buildEvolutionBuckets(period: PeriodResolved): Array<{ label: string; start: Date; end: Date }> {
...
type Scope = 'global' | 'lotissement' | 'programme' | 'owner';
 ...
 */
function invoiceScopeWhere(scope: Scope, scopeId?: number): any {
 ...
 */
async function treasuryScopeWhere(db: ReturnType<typeof getDb>, scope: Scope, scopeId?: number): Promise<any> {
  if (scope === 'global' || !scopeId) return {};
  if (scope === 'lotissement') return { lotissementId: scopeId };
  if (scope === 'programme')   return { programmeId: scopeId };
  ...
  if (paymentIds.length === 0 && installmentIds.length === 0) return { id: -1 };
  return {
    OR: [
...
async function resolveScopeLabel(db: ReturnType<typeof getDb>, scope: Scope, scopeId?: number): Promise<string> {
  if (scope === 'global' || !scopeId) return 'Tous périmètres confondus';
  if (scope === 'lotissement') {
  ...
  if (!row) return `Propriétaire #${scopeId}`;
...
// ── Calcul recettes / dépenses sur une période ───────────────────────────────
...
// ── Calcul Actif / Passif à une date donnée ──────────────────────────────────

interface ActifPassif {
  asOfDate: string;
  scope: { type: Scope; id?: number; label: string };
  actif: {
...
async function computeActifPassif(
  db: ReturnType<typeof getDb>,
  asOfDate: Date,
  scope: Scope,
  scopeId?: number,
  ...
  const propertyWhere: any = { deletedAt: null, status: { not: 'VENDU' } };
  const terrainWhere: any = { deletedAt: null, statut: { not: 'VENDU' } };
  ...
  const [propsList, terrainsList] = await Promise.all([
    scope === 'lotissement' && scopeId
      ? Promise.resolve([] as Array<{ reference: string; salePrice: any }>)
  ...
  const immobilisations: Array<{ label: string; montant: number }> = [];
  ...
  if (sumProps > 0) { immobilisations.push({ label: `Biens immobiliers (${propsList.length})`, montant: sumProps }); totalImmobilisations += sumProps; }
  if (sumTerr  > 0) { immobilisations.push({ label: `Terrains (${terrainsList.length})`,        montant: sumTerr  }); totalImmobilisations += sumTerr;  }
  ...
  const creancesMap: Record<string, number> = {};
  ...
  const echeancesMap: Record<string, number> = { 'À recevoir': 0, 'En retard': 0 };
  for (const it of installments) {
    if (it.status === 'EN_RETARD') echeancesMap['En retard'] += Number(it.amount);
    else                            echeancesMap['À recevoir'] += Number(it.amount);
  }
  ...
  const tresorerie: Array<{ label: string; montant: number }> = [];
  ...
  let dettesProprietaires: Array<{ label: string; montant: number }> = [];
...
const periodTypeSchema = z.enum(['month', 'last-month', 'quarter', 'semester', 'year', 'custom']);
const scopeSchema = z.enum(['global', 'lotissement', 'programme', 'owner']);

const resultatPayloadSchema = z.object({
...
const actifPassifPayloadSchema = z.object({
...
const exportPayloadSchema = z.object({
  type:   z.enum(['resultat', 'actif-passif']),
...
async function getCompanyHeader(): Promise<{ name: string; address: string; logoBase64?: string; logoMime?: string }> {
  const map = await getSettings([
    SettingsKeys.companyName, SettingsKeys.companyAddress, SettingsKeys.companyLogo,
  ]);
  const out: { name: string; address: string; logoBase64?: string; logoMime?: string } = {
...
function buildResultatHtml(data: ResultatData, meta: { title: string; subtitle?: string }, company: Awaited<ReturnType<typeof getCompanyHeader>>, theme: ThemePalette): string {
...
interface ActifPassifExportData extends ActifPassif {
  /** Reprend la structure ActifPassif — types portés au niveau export. */
}

function buildActifPassifHtml(data: ActifPassifExportData, meta: { title: string; subtitle?: string }, company: Awaited<ReturnType<typeof getCompanyHeader>>, theme: ThemePalette): string {
  const primary = theme.primary;
  const accent  = theme.accent;
  const muted   = theme.textMuted;
  const surface = theme.surface;
  ...
  const section = (title: string, rows: Array<{ label: string; montant: number }>, sectionTotal: number) => {
...
async function buildXlsxResultat(data: ResultatData, meta: { title: string; subtitle?: string }, theme: ThemePalette): Promise<Buffer> {
  ...
  ws.getCell('A5').value = 'KPI';        ws.getCell('B5').value = 'Montant (XOF)';
  ...
  ws.getCell('A6').value = 'Total recettes'; ws.getCell('B6').value = data.totalRecettes;
  ws.getCell('A7').value = 'Total dépenses'; ws.getCell('B7').value = data.totalDepenses;
  ws.getCell('A8').value = 'Résultat net';   ws.getCell('B8').value = data.resultat;
...
async function buildXlsxActifPassif(data: ActifPassifExportData, meta: { title: string; subtitle?: string }, theme: ThemePalette): Promise<Buffer> {
  const NAVY = hexToArgb(theme.primary);
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Afrikimmo-App';
  wb.created = new Date();
  ...
  let row = 5;
  const writeHeader = (left: string, right: string) => {
  ...
  const writeRow = (left: string, leftAmt: number | '', right: string, rightAmt: number | '', section = false) => {
  ...
  const maxLen = Math.max(
    actifSections.reduce((s, sec) => s + sec.items.length + 1, 0),
    passifSections.reduce((s, sec) => s + sec.items.length + 1, 0),
  );

  // Aplatit chaque colonne en lignes (titre de section + items)
  const flat = (sections: Array<{ title: string; items: Array<{ label: string; montant: number }>; total: number; mono?: boolean }>) => {
    const out: Array<[string, number | '', boolean]> = [];
    for (const sec of sections) {
      out.push([sec.title, sec.total, true]);
      if (!sec.mono) {
        for (const it of sec.items) out.push([`  ${it.label}`, it.montant, false]);
      }
    }
    while (out.length < maxLen) out.push(['', '', false]);
    return out;
  };
...
export function registerBilanIPC(): void {
  ipcMain.handle('bilan:getResultat', async (_event, payload: any) => {
    try {
      ...
      logger.error('bilan:getResultat error', err.message);
      return { success: false, error: err.message };
    }
  });

  ipcMain.handle('bilan:getActifPassif', async (_event, payload: any) => {
    try {
      const session = getSession(payload?.token);
      if (!session) return { success: false, error: 'Session expirée' };
      ...
      if (Number.isNaN(asOfDate.getTime())) return { success: false, error: 'Date d\'arrêté invalide' };
      ...
      logger.error('bilan:getActifPassif error', err.message);
      return { success: false, error: err.message };
    }
  });

  ipcMain.handle('bilan:export', async (_event, payload: any) => {
    try {
      const session = getSession(payload?.token);
      if (!session) return { success: false, error: 'Session expirée' };
      checkBilanRole(session);
      ...
      if (parsed.data.type === 'actif-passif') checkActifPassifRole(session);
      ...
      const dialogRes = await dialog.showSaveDialog(parent!, {
      ...
      const [theme, company] = await Promise.all([
        getThemeForUser(session.userId),
        getCompanyHeader(),
      ]);

      let fileBuffer: Buffer;
      if (parsed.data.type === 'resultat') {
        const data = parsed.data.data as ResultatData;
        if (parsed.data.format === 'pdf') {
          const html = buildResultatHtml(data, parsed.data.meta, company, theme);
          fileBuffer = await htmlToPdf(html, { landscape: false });
        } else {
          fileBuffer = await buildXlsxResultat(data, parsed.data.meta, theme);
        }
      } else {
        const data = parsed.data.data as ActifPassifExportData;
        if (parsed.data.format === 'pdf') {
          const html = buildActifPassifHtml(data, parsed.data.meta, company, theme);
          fileBuffer = await htmlToPdf(html, { landscape: true });
        } else {
          fileBuffer = await buildXlsxActifPassif(data, parsed.data.meta, theme);
        }
      }

      fs.writeFileSync(dialogRes.filePath, fileBuffer);
      logger.info(`Export bilan ${parsed.data.type}/${ext} : ${dialogRes.filePath}`);
      return { success: true, data: { path: dialogRes.filePath } };
    } catch (err: any) {
      logger.error('bilan:export error', err.message);
      return { success: false, error: err.message };
    }
  });
}
