 ...
 */
export function diffChangedFields(before: Record<string, any>, data: Record<string, any>): string[] {
  const norm = (v: unknown): unknown => {
    if (v == null) return null;
    if (v instanceof Date) return v.toISOString();
    if (typeof v === 'object' && typeof (v as any).toString === 'function') return (v as any).toString();
    return v;
  };
  const changed: string[] = [];
  for (const key of Object.keys(data)) {
    if (data[key] === undefined) continue;
    if (norm(before[key]) !== norm(data[key])) changed.push(key);
  }
  return changed;
}
...
export async function recordStatusChange(
  ...
  if (!p.oldValue || p.oldValue === p.newValue) return;
...
export async function recordModification(
  ...
  if (!p.changedFields.length) return;
