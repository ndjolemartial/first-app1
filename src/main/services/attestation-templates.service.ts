 ...
 */
export async function seedDefaultAttestationTemplate(): Promise<void> {
  ...
  if (existing) return; // un modèle SOLDE existe déjà → on ne modifie rien
  ...
  logger.info('Default SOLDE attestation template created');
}
