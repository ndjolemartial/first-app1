...
const TEMPLATES: Record<string, string> = {
  CDI:
    art('Article 1 — Engagement',
      `{{entreprise.nom}} engage {{employe.civilite}} {{employe.nomComplet}} en qualité de <strong>{{contrat.poste}}</strong> (catégorie {{contrat.categorie}}), dans le cadre d'un contrat de travail à durée indéterminée régi par la Loi n°2015-532 portant Code du travail de la République de Côte d'Ivoire et la convention collective applicable.`) +
    art('Article 2 — Prise d\'effet et période d\'essai',
      `Le présent contrat prend effet à compter du <strong>{{contrat.dateDebut}}</strong>. Il est assorti d'une période d'essai expirant le {{contrat.finEssai}}, durant laquelle chacune des parties peut rompre le contrat sans préavis ni indemnité, conformément à la réglementation en vigueur.`) +
    ...
    art('Article 5 — Rémunération',
      `En contrepartie de son travail, le salarié percevra un salaire de base mensuel brut de <strong>{{contrat.salaireBase}}</strong>, payable à terme échu, sous déduction des cotisations sociales (CNPS) et fiscales (ITS) légales.`) +
    ...
    art('Article 7 — Rupture',
      `La rupture du présent contrat obéit aux dispositions du Code du travail relatives au préavis, au licenciement et à la démission.`),

  CDD:
    art('Article 1 — Engagement et durée',
      `{{entreprise.nom}} engage {{employe.civilite}} {{employe.nomComplet}} en qualité de <strong>{{contrat.poste}}</strong> dans le cadre d'un contrat à durée déterminée, conclu du <strong>{{contrat.dateDebut}}</strong> au <strong>{{contrat.dateFin}}</strong>, conformément à la Loi n°2015-532 portant Code du travail.`) +
    art('Article 2 — Motif et période d\'essai',
      `Le présent contrat est conclu pour faire face à un besoin déterminé de l'entreprise. Il comporte une période d'essai expirant le {{contrat.finEssai}}.`) +
    art('Article 3 — Fonctions et durée du travail',
      `Le salarié exercera les fonctions de {{contrat.poste}} (catégorie {{contrat.categorie}}), pour une durée hebdomadaire de <strong>{{contrat.heuresHebdo}} heures</strong>.`) +
    art('Article 4 — Rémunération',
      `Le salaire de base mensuel brut est fixé à <strong>{{contrat.salaireBase}}</strong>, sous déduction des cotisations CNPS et de l'ITS.`) +
    art('Article 5 — Fin du contrat',
      `Le contrat prend fin de plein droit à l'arrivée du terme convenu, sans préavis. Le renouvellement éventuel s'effectue dans les limites fixées par le Code du travail.`) +
    art('Article 6 — Sécurité sociale',
      `Le salarié est affilié à la CNPS sous le numéro {{employe.cnps}}.`),

  STAGE:
    art('Article 1 — Objet',
      `La présente convention a pour objet l'accueil de {{employe.civilite}} {{employe.nomComplet}} en qualité de stagiaire au poste de <strong>{{contrat.poste}}</strong> au sein de {{entreprise.nom}}, à des fins de formation pratique.`) +
    art('Article 2 — Durée',
      `Le stage se déroule du <strong>{{contrat.dateDebut}}</strong> au <strong>{{contrat.dateFin}}</strong>, pour une durée hebdomadaire de {{contrat.heuresHebdo}} heures.`) +
    art('Article 3 — Gratification',
      `Le stagiaire perçoit une gratification mensuelle de <strong>{{contrat.salaireBase}}</strong>. La présente convention ne constitue pas un contrat de travail.`) +
    art('Article 4 — Encadrement et obligations',
      `Le stagiaire est placé sous la responsabilité d'un maître de stage et s'engage à respecter le règlement intérieur et la confidentialité des informations de l'entreprise.`),

  INTERIM:
    art('Article 1 — Objet et durée',
      `{{entreprise.nom}} engage {{employe.civilite}} {{employe.nomComplet}} dans le cadre d'un contrat de travail temporaire au poste de <strong>{{contrat.poste}}</strong>, du <strong>{{contrat.dateDebut}}</strong> au <strong>{{contrat.dateFin}}</strong>.`) +
    art('Article 2 — Mission',
      `Le salarié est mis à disposition pour l'accomplissement d'une mission temporaire correspondant à la catégorie {{contrat.categorie}}, pour {{contrat.heuresHebdo}} heures hebdomadaires.`) +
    art('Article 3 — Rémunération',
      `La rémunération mensuelle brute est fixée à <strong>{{contrat.salaireBase}}</strong>, soumise aux retenues CNPS et ITS.`) +
    art('Article 4 — Fin de mission',
      `Le contrat prend fin à l'échéance de la mission, conformément aux dispositions légales applicables au travail temporaire.`),

  CONSULTANT:
    art('Article 1 — Objet',
      `{{entreprise.nom}} confie à {{employe.civilite}} {{employe.nomComplet}} une mission de prestation de services / consultance en qualité de <strong>{{contrat.poste}}</strong>.`) +
    art('Article 2 — Durée',
      `La mission est exécutée du <strong>{{contrat.dateDebut}}</strong> au <strong>{{contrat.dateFin}}</strong>.`) +
    art('Article 3 — Honoraires',
      `En contrepartie de ses prestations, le consultant perçoit une rémunération mensuelle de <strong>{{contrat.salaireBase}}</strong>. Le consultant fait son affaire personnelle de ses obligations fiscales et sociales, sauf disposition contraire.`) +
    art('Article 4 — Indépendance et confidentialité',
      `Le consultant exécute sa mission en toute indépendance et s'engage à une stricte confidentialité sur les informations dont il a connaissance.`),

  APPRENTISSAGE:
    art('Article 1 — Objet',
      `{{entreprise.nom}} s'engage à donner à {{employe.civilite}} {{employe.nomComplet}} une formation professionnelle méthodique et complète au métier de <strong>{{contrat.poste}}</strong>, dans le cadre d'un contrat d'apprentissage.`) +
    art('Article 2 — Durée',
      `L'apprentissage se déroule du <strong>{{contrat.dateDebut}}</strong> au <strong>{{contrat.dateFin}}</strong>, à raison de {{contrat.heuresHebdo}} heures hebdomadaires.`) +
    art('Article 3 — Allocation',
