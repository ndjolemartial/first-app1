...
export async function getWorkHoursPerDay(): Promise<number> {
  const row = await getDb().appSetting.findUnique({ where: { key: HOURS_PER_DAY_KEY } });
  const v = Number(row?.value);
  return Number.isFinite(v) && v > 0 ? v : DEFAULT_HOURS_PER_DAY;
}

async function getAttendanceSettings(): Promise<{ monthlyHours: number; overtimeMajoration: number }> {
  const db = getDb();
 ...
 */
export async function attendanceMonthSummary(employeeId: number, year: number, month: number): Promise<AttendanceMonthSummary> {
  const db = getDb();
  ...
  for (const r of records) {
    ...
    else if (r.status === 'CONGE' || r.status === 'MALADIE') daysLeave += 1;
...
export async function computeOvertimeAmount(baseSalary: number, overtimeHours: number): Promise<number> {
  if (!(overtimeHours > 0) || !(baseSalary > 0)) return 0;
