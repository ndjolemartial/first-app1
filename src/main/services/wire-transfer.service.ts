...
import { hexToArgb } from './theme.service';
import { moneyToFrenchWords } from '../utils/numberToWords';

/** En-têtes fixes des 8 colonnes du tableau (ordre non modifiable, largeurs éditables). */
export const WIRE_TRANSFER_COLUMN_HEADERS = [
...
export const DEFAULT_WIRE_TRANSFER_COLUMN_WIDTHS = [5, 24, 11, 11, 17, 8, 14, 10];

/** Bloc d'introduction par défaut (4 lignes), avec variables `{{...}}` résolues à la génération. */
export const DEFAULT_WIRE_TRANSFER_INTRO_HTML =
  '<p>Nous vous prions de bien vouloir procéder au virement des salaires du personnel ci-dessous énuméré.</p>' +
...
export interface WireTransferTemplateOpts {
...
export interface WireTransferBeneficiary {
  fullName: string;
  bankCode: string;
...
const fmtAmount = (n: unknown) => new Intl.NumberFormat('fr-FR').format(Math.round(Number(n ?? 0)));
const todayFr = () => new Date().toLocaleDateString('fr-FR');

/** Résout le bloc d'introduction (HTML, variables `{{...}}` remplacées) — partagé PDF/Excel. */
function resolveIntroHtml(template: WireTransferTemplateOpts, periodLabel: string, beneficiaryCount: number, totalNet: number): string {
  const introRaw = (template.introHtml && template.introHtml.trim()) || DEFAULT_WIRE_TRANSFER_INTRO_HTML;
...
function htmlToTextLines(html: string | null | undefined): string[] {
  if (!html) return [];
...
function parseImageDataUri(dataUri: string | null | undefined): { base64: string; extension: 'png' | 'jpeg' | 'gif' } | null {
...
export function renderWireTransferOrderHtml(
  beneficiaries: WireTransferBeneficiary[],
  periodLabel: string,
  logoDataUri: string | null,
  template: WireTransferTemplateOpts,
): string {
  const esc = (v: unknown) =>
  ...
  const widths =
    Array.isArray(template.columnWidths) && template.columnWidths.length === 8
      ...
      : DEFAULT_WIRE_TRANSFER_COLUMN_WIDTHS;
  ...
  const thead = WIRE_TRANSFER_COLUMN_HEADERS
...
<html lang="fr"><head><meta charset="utf-8"><style>
  * { box-sizing: border-box; }
  ...
  <div class="amount-words">Arrêté le présent ordre de virement à la somme de : <strong>${esc(moneyToFrenchWords(totalNet))}</strong>.</div>
  <div class="signature">
    <div class="box">
...
export async function buildWireTransferXlsx(
  beneficiaries: WireTransferBeneficiary[],
  periodLabel: string,
  template: WireTransferTemplateOpts,
  logoDataUri?: string | null,
): Promise<Buffer> {
  const NAVY = hexToArgb((template.accentColor && template.accentColor.trim()) || '#1E3A5F');
  const headers = WIRE_TRANSFER_COLUMN_HEADERS;
  ...
  for (let ci = 1; ci <= colCount; ci++) ws.getRow(r).getCell(ci).border = border;
  r++;

  ws.mergeCells(r, 1, r, colCount);
  const amountWordsCell = ws.getCell(r, 1);
  amountWordsCell.value = `Arrêté le présent ordre de virement à la somme de : ${moneyToFrenchWords(totalNet)}.`;
