...
async function main() {
    ...
    const { applyReminderRules } = await Promise.resolve().then(() => __importStar(require('../services/reminders.service')));
    ...
    try {
        ...
        logger.info(`[run-reminders-once] scanned=${result.scanned} sent=${result.sent} ` +
