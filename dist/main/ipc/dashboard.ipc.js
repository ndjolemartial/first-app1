    ...
    electron_1.ipcMain.handle('dashboard:getStats', async (_event, { token }) => {
        try {
            ...
            const attendanceQrPromise = db.appSetting
                ...
                if (map.get(ATT_QR_ENABLED_KEY) !== 'true')
                    return null;
                ...
                let roles = [];
                try {
                    const p = JSON.parse(map.get(ATT_QR_ROLES_KEY) ?? '[]');
                    if (Array.isArray(p))
                        roles = p;
                }
                catch { /* noop */ }
            ...
            const visitorQrPromise = db.appSetting
                ...
                .then((rows) => {
                const map = new Map(rows.map((r) => [r.key, r.value]));
                if (map.get(VIS_QR_ENABLED_KEY) !== 'true')
                    return null;
                ...
                let roles = [];
                try {
                    const p = JSON.parse(map.get(VIS_QR_ROLES_KEY) ?? '[]');
                    if (Array.isArray(p))
                        roles = p;
                }
                catch { /* noop */ }
