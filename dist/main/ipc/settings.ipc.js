...
function registerSettingsIPC() {
     ...
     */
    function assertLatenessManagerWrite(session) {
    ...
    electron_1.ipcMain.handle('settings:updateLatenessSettings', async (_event, { token, payload }) => {
        try {
            ...
            if (ADMIN_ROLES.includes(session.role)) {
                updates.push({ key: settings_service_1.SettingsKeys.latenessIncludeManagementRoles, value: parsed.data.includeManagementRoles ? 'true' : 'false' });
            }
            await (0, settings_service_1.setSettings)(updates);
            logger_1.default.info(`Retards & Départs précipités — tolérance : ${parsed.data.toleranceMinutes} min${ADMIN_ROLES.includes(session.role) ? `, inclusion SUPER_ADMIN/ADMIN/MANAGER : ${parsed.data.includeManagementRoles}` : ''}`);
            return { success: true };
        }
        catch (err) {
     ...
     */
    electron_1.ipcMain.handle('settings:listAttendanceSpecialDays', async (_event, { token }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            logger_1.default.error('settings:listAttendanceSpecialDays', err.message);
            return { success: false, error: err.message };
        }
    });
    electron_1.ipcMain.handle('settings:createAttendanceSpecialDay', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            assertLatenessManagerWrite(session);
            const schema = zod_1.z.object({
                date: zod_1.z.string().min(1, 'Date requise'),
                expectedDeparture: zod_1.z.string().regex(HHMM_RE, 'Heure de départ invalide (HH:MM)'),
                expectedArrival: zod_1.z.string().regex(HHMM_RE, "Heure d'arrivée invalide (HH:MM)").nullable().optional(),
                label: zod_1.z.string().max(191).nullable().optional(),
            });
            const parsed = schema.safeParse(payload);
            if (!parsed.success)
                return { success: false, error: parsed.error.issues.map((i) => i.message).join(', ') };
            ...
            if (existing)
                return { success: false, error: 'Une journée à horaire réduit existe déjà pour cette date.' };
            ...
            logger_1.default.info(`Journée à horaire réduit ajoutée : ${d.date} (départ ${d.expectedDeparture})`);
            return { success: true, data: created };
        }
        catch (err) {
            logger_1.default.error('settings:createAttendanceSpecialDay', err.message);
            return { success: false, error: err.message };
        }
    });
    electron_1.ipcMain.handle('settings:deleteAttendanceSpecialDay', async (_event, { token, id }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            logger_1.default.error('settings:deleteAttendanceSpecialDay', err.message);
            return { success: false, error: err.message };
        }
    });
