...
const invoiceSchema = zod_1.z.object({
    type: zod_1.z.enum(['VENTE', 'ECHEANCE_VENTE', 'FRAIS_AGENCE', 'FRAIS_DE_GESTION', 'AVANCE', 'CAUTION', 'OTHER']),
    clientId: zod_1.z.number().int().optional(),
    contractId: zod_1.z.number().int().optional(),
...
function registerAccountingIPC() {
    ...
    electron_1.ipcMain.handle('accounting:getDashboard', async (_event, { token }) => {
        try {
            ...
            const [invoiceStats, overdueInstallments, upcomingInstallments, activeContracts, monthlyRevenue,] = await db.$transaction([
        ...
        try {
            ...
            const invoice = await db.invoice.create({
                data: {
                    ...
                    notes: d.notes,
                    items: { create: d.items.map((item) => ({ ...item, quantity: item.quantity, unitPrice: item.unitPrice, total: item.total })) },
                },
                include: { items: true },
            });
    ...
    electron_1.ipcMain.handle('accounting:addPayment', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
    ...
    /* ─── Échéances de vente ─────────────────────────────────────────── */
    electron_1.ipcMain.handle('accounting:getOverdueInstallments', async (_event, { token, page = 1, limit = 20 }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
    ...
    electron_1.ipcMain.handle('accounting:getUpcomingInstallments', async (_event, { token, days = 30, page = 1, limit = 20 }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
    ...
    electron_1.ipcMain.handle('accounting:payInstallment', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            logger_1.default.info(`Installment paid: id=${d.installmentId} contract=${installment.contract.reference}`);
    ...
    electron_1.ipcMain.handle('accounting:getSaleContracts', async (_event, { token, page = 1, limit = 20 }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
