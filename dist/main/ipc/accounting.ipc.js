...
async function loadCompanyEmitter() {
...
function buildInvoiceHtml(inv, tpl, company = null) {
    ...
    const sectionBg = tpl?.sectionColor || '#d7dfe8';
    const tableHeaderBg = tpl?.tableHeaderColor || '#e2e8f0';
    const sectionFg = tpl?.sectionTextColor || '#0f172a';
    const tableHeaderFg = tpl?.tableHeaderTextColor || '#0f172a';
    ...
    const endOfDocument = tpl?.endOfDocument || '';
    ...
    const emitterLines = [];
    if (company?.phoneFixed)
        emitterLines.push(`Tél : ${company.phoneFixed}`);
    if (company?.address) {
        for (const line of company.address.split(/\r?\n/).filter(Boolean))
            emitterLines.push(line);
    }
    if (company?.registre)
        emitterLines.push(`RCCM : ${company.registre}`);
    const c = inv.client;
    const clientName = c
    ...
    const INSTALLMENT_STATUS_FR = {
        PAYE: 'Payé', PARTIEL: 'Partiel', EN_ATTENTE: 'En attente',
        A_REGLER: 'À régler', EN_RETARD: 'En retard', ANNULE: 'Annulé',
    };
    const v = inv.versement;
    const coveredRows = v
        ? v.covered.map((c) => `<tr><td>n°${c.number}</td><td>${fmtDate(c.dueDate)}</td><td class="num">${fmt(c.amount)}</td><td class="num">${fmt(c.applied)}</td><td class="num">${fmt(c.paidAmount)}</td><td>${esc(INSTALLMENT_STATUS_FR[c.status] ?? c.status)}</td></tr>`).join('')
        : '';
    const remainingRows = v
        ? v.remaining.map((r) => `<tr><td>n°${r.number}</td><td>${fmtDate(r.dueDate)}</td><td class="num">${fmt(r.amount)}</td><td class="num">${fmt(r.paidAmount)}</td><td class="num">${fmt(r.remaining)}</td></tr>`).join('')
        : '';
    return `<!DOCTYPE html>
...
function registerAccountingIPC() {
    ...
    electron_1.ipcMain.handle('accounting:getDashboard', async (_event, { token }) => {
        try {
            ...
            const unpaidInstallmentWhere = {
                status: { in: ['EN_ATTENTE', 'A_REGLER', 'EN_RETARD', 'PARTIEL'] },
                conventionId: { not: null },
            };
            const [paidInvoicesAgg, overdueRows, unpaidRows, upcomingInstallments, overdueInstallments, recentInvoices, unpaidFull, partialInvoices, partialPayments,] = await Promise.all([
            ...
            const remaining = (r) => Number(r.amount) - Number(r.paidAmount);
    ...
    electron_1.ipcMain.handle('accounting:getInvoiceById', async (_event, { token, id }) => {
        try {
            ...
            if (!invoice.conventionId) {
                ...
                if (details)
                    invoice.detailsSouscription = details;
            }
        ...
        try {
            ...
            if ((invoice.type === 'VENTE' || invoice.type === 'APPORT_INITIAL') && invoice.conventionId && invoice.convention) {
                try {
                    ...
                    const tr = (0, commission_service_1.commissionTypeAndRate)(invoice.convention.type, rates);
                    const beneficiaryUserId = invoice.convention.agentId ?? invoice.convention.client?.assignedToId ?? null;
                    if (tr && beneficiaryUserId) {
                        await (0, commission_service_1.accrueCollectionCommission)(db, {
                    ...
                    logger_1.default.error('accrueCollectionCommission (facture) error', e?.message ?? e);
                }
            }
    ...
    electron_1.ipcMain.handle('accounting:printInvoice', async (_event, { token, invoiceId }) => {
        try {
            ...
            if (invoice.client?.country) {
                ...
                if (country)
                    invoice.client.countryName = country.name;
            }
            ...
            const anchorTime = anchor ? +new Date(anchor.paidAt) : null;
            ...
            const paidAsOf = (i, asOfTime) => round2((i.invoice?.payments ?? [])
                .filter((p) => +new Date(p.paidAt) <= asOfTime)
                .reduce((s, p) => s + Number(p.amount), 0));
            const buildRecap = (active) => {
                if (active.length === 0)
                    return null;
                if (anchorTime == null) {
                ...
                for (const i of active) {
                    ...
                    if (settled + 0.001 >= Number(i.amount)) {
                        paidCount += 1;
                        paidAmount += Number(i.amount);
                    }
                    else {
                        remainingCount += 1;
                        remainingAmount += round2(Number(i.amount) - settled);
                    }
                }
            ...
            const buildVersement = (active) => {
                if (!anchor || anchorTime == null || invoice.type !== 'ECHEANCE_VENTE')
                    return null;
                const sameVersement = (p) => +new Date(p.paidAt) === anchorTime &&
            ...
            if (recapByConvention && invoice.conventionId) {
                ...
                const terrainKey = (i) => {
                    const ids = (i.terrainLinks ?? []).map((l) => l.terrain?.id).filter(Boolean);
                    if (ids.length === 0 && i.terrainId)
                        ids.push(i.terrainId);
            ...
            const pdf = await (0, pdf_service_1.htmlToPdf)(buildInvoiceHtml({ ...invoice, installmentRecap, versement }, template, company), { landscape: false });
            ...
            await (0, pdf_service_1.openPrintPreview)(pdf, `Facture ${invoice.reference}`);
            logger_1.default.info(`Aperçu impression facture: ${invoice.reference}`);
    ...
    electron_1.ipcMain.handle('accounting:listInstallments', async (_event, { token, crmReferentScope }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            const db = (0, db_service_1.getDb)();
            const where = { conventionId: { not: null } };
            if (scoped) {
    ...
    electron_1.ipcMain.handle('accounting:getPaidInstallments', async (_event, { token, year = 0, semester = 0 }) => {
        try {
            ...
            checkAccountingRole(session, READ_ROLES);
            const db = (0, db_service_1.getDb)();
            const where = { status: 'PAYE', conventionId: { not: null } };
    ...
    electron_1.ipcMain.handle('accounting:payInstallment', async (_event, { token, installmentId, payload }) => {
        try {
            ...
            if (installment.status === 'ANNULE')
                return { success: false, error: 'Échéance annulée' };
            ...
            if (!(currentRemaining > 0))
                return { success: false, error: 'Échéance déjà soldée' };
            ...
            if (payAmount <= 0)
                return { success: false, error: 'Montant à encaisser invalide' };
            const labelOf = (i) => installment.convention
                ? `convention ${installment.convention.reference}`
            ...
            if (payAmount - currentRemaining > 0.001) {
                if (installment.conventionId) {
                    ...
                    const terrainKey = (i) => {
                        const ids = (i.terrainLinks ?? []).map((l) => l.terrainId).filter(Boolean);
                        if (ids.length === 0 && i.terrainId)
                            ids.push(i.terrainId);
                        return [...ids].sort((a, b) => a - b).join(',');
                    };
            ...
            const chain = [toChain(installment), ...siblings.map(toChain)];
            ...
            logger_1.default.info(`Installment payment: id=${d.installmentId} +${payAmount} réparti sur ${coveredCount} échéance(s) ` +
            ...
            if (installment.conventionId && installment.convention) {
                try {
                    const rates = await (0, commission_service_1.getDefaultRates)(db);
                    const tr = (0, commission_service_1.commissionTypeAndRate)(installment.convention.type, rates);
                    const beneficiaryUserId = installment.convention.agentId ?? installment.convention.client?.assignedToId ?? null;
                    if (tr && beneficiaryUserId) {
                    ...
                    logger_1.default.error('accrueCollectionCommission (échéance) error', e?.message ?? e);
                }
            }
            return ser({
                success: true,
    ...
    electron_1.ipcMain.handle('accounting:getLegacyInstallments', async (_event, { token }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
    ...
    electron_1.ipcMain.handle('accounting:updateLegacyInstallment', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            const schema = zod_1.z.object({
                installmentId: zod_1.z.number().int().positive(),
                detailsSouscription: zod_1.z.string().nullable().optional(),
                dueDate: zod_1.z.string().min(1, 'Date requise'),
            ...
            if (terrainIds.length > 0) {
                ...
                if (found !== terrainIds.length)
                    return { success: false, error: 'Un terrain sélectionné est introuvable' };
            }
            ...
            logger_1.default.info(`Échéance héritée modifiée: id=${d.installmentId} terrains=[${terrainIds.join(',')}]`);
