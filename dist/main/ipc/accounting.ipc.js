    ...
    electron_1.ipcMain.handle('accounting:printInvoice', async (_event, { token, invoiceId }) => {
        try {
            ...
            if (recapByConvention && invoice.conventionId) {
                ...
                const linksInclude = { terrainLinks: { select: { terrain: { select: { id: true } } } } };
                ...
                if (paidInst && cId) {
                    ...
                    installmentRecap = buildRecap(all.filter((i) => i.status !== 'ANNULE' && terrainKey(i) === key));
                }
            }
            ...
            const pdf = await (0, pdf_service_1.htmlToPdf)(buildInvoiceHtml({ ...invoice, installmentRecap }, template, company), { landscape: false });
