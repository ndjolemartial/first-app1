...
const createUserSchema = zod_1.z.object({
    ...
    role: zod_1.z.enum(['SUPER_ADMIN', 'ADMIN', 'MANAGER', 'ACCOUNTANT', 'ASSISTANTE_DIRECTION', 'AGENT', 'AGENT_TECHNIQUE', 'RH', 'READONLY']),
    ...
    defaultAccountEntreeId: zod_1.z.number().int().positive().nullable().optional(),
    defaultAccountSortieId: zod_1.z.number().int().positive().nullable().optional(),
});
 ...
 */
function selectableUserRoles(role) {
    switch (role) {
        case 'SUPER_ADMIN':
        case 'ADMIN':
...
function registerUsersIPC() {
     ...
     */
    electron_1.ipcMain.handle('users:listSelectable', async (_event, { token, options = {} }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            logger_1.default.error('users:listSelectable error', error.message);
            return { success: false, error: error.message };
        }
    });
