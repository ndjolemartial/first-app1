...
exports.registerProformaIPC = registerProformaIPC;
...
function canAccessRecord(session, createdById) {
    return FULL_ACCESS_ROLES.includes(session.role) || createdById === session.userId;
}
/** Nom d'affichage d'un client (particulier ou entreprise). */
function clientLabel(cl) {
    if (!cl)
        return '';
...
async function nextProformaReference(db) {
...
function registerProformaIPC() {
    electron_1.ipcMain.handle('proforma:list', async (_event, { token, filters = {}, page = 1, limit = 20 }) => {
        try {
            ...
            logger_1.default.error('proforma:list error', error.message);
            return { success: false, error: error.message };
        }
    });
    electron_1.ipcMain.handle('proforma:getById', async (_event, { token, id }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            if (!proforma)
                return { success: false, error: 'Facture Proforma introuvable' };
            if (!canAccessRecord(session, proforma.createdById))
                return { success: false, error: 'Facture Proforma inaccessible' };
            return ser({ success: true, data: proforma });
        }
        catch (error) {
            logger_1.default.error('proforma:getById error', error.message);
            return { success: false, error: error.message };
        }
    });
    electron_1.ipcMain.handle('proforma:createFromQuote', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            const terrainsList = quote.terrains.length ? quote.terrains.map((l) => l.terrain) : (quote.terrain ? [quote.terrain] : []);
            const propertiesList = quote.properties.length ? quote.properties.map((l) => l.property) : (quote.property ? [quote.property] : []);
            const terrainDesc = terrainsList.map((t) => `${t.reference}${t.lotissement?.nom ? ` (${t.lotissement.nom})` : ''}`).join(', ');
            const propertyDesc = propertiesList.map((p) => `${p.reference}${p.address ? ` — ${p.address}` : ''}`).join(', ');
            ...
            const items = quote.items
                .filter((it) => it.lineType === 'ARTICLE' && Number(it.quantity) > 0)
                .map((it) => ({
            ...
            const created = await db.proformaInvoice.create({
                data: {
                    ...
                    designation, items: items,
                    subtotal: dec(subtotal),
                    taxRate: dec(Number(quote.taxRate ?? 0)),
                    taxAmount: dec(Number(quote.taxAmount ?? 0)),
                    total: dec(Number(quote.total ?? 0)),
                    validUntil: d.validUntil ? new Date(d.validUntil) : null,
                    notes: d.notes?.trim() || null,
                    createdById: session.userId,
                },
            });
            logger_1.default.info(`Facture Proforma générée depuis le devis ${quote.reference} : ${reference}`);
            return ser({ success: true, data: created });
        }
        catch (error) {
            logger_1.default.error('proforma:createFromQuote error', error.message);
            return { success: false, error: error.message };
        }
    });
    electron_1.ipcMain.handle('proforma:createFromConvention', async (_event, { token, payload }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            const terrainsList = convention.terrains.map((l) => l.terrain);
            const propertiesList = convention.properties.map((l) => l.property);
            const terrainDesc = terrainsList.map((t) => `${t.reference}${t.lotissement?.nom ? ` (${t.lotissement.nom})` : ''}`).join(', ');
            const propertyDesc = propertiesList.map((p) => `${p.reference}${p.address ? ` — ${p.address}` : ''}`).join(', ');
            ...
            const items = [{ designation: designation || convention.reference, quantity: 1, unit: null, unitPrice: amount, total: amount }];
            ...
            logger_1.default.info(`Facture Proforma générée depuis la convention ${convention.reference} : ${reference}`);
            return ser({ success: true, data: created });
        }
        catch (error) {
            logger_1.default.error('proforma:createFromConvention error', error.message);
            return { success: false, error: error.message };
        }
    });
    electron_1.ipcMain.handle('proforma:delete', async (_event, { token, id }) => {
        try {
            const session = (0, auth_service_1.getSession)(token);
            if (!session)
                return { success: false, error: 'Session expirée' };
            ...
            logger_1.default.error('proforma:delete error', error.message);
            return { success: false, error: error.message };
        }
    });
}
