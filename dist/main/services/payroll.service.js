...
exports.loadPayslipLogo = loadPayslipLogo;
exports.getPayrollRates = getPayrollRates;
exports.setPayrollRates = setPayrollRates;
exports.computeIts = computeIts;
exports.ricfMonthlyForParts = ricfMonthlyForParts;
exports.seniorityRate = seniorityRate;
exports.computePrimeAnciennete = computePrimeAnciennete;
exports.computePayroll = computePayroll;
exports.computePayslipTotals = computePayslipTotals;
exports.loadPayslipCompany = loadPayslipCompany;
...
async function loadPayslipLogo() {
    try {
...
async function getPayrollRates(db = (0, db_service_1.getDb)()) {
...
async function setPayrollRates(rates, db = (0, db_service_1.getDb)()) {
...
function computeIts(taxable, brackets) {
    let tax = 0;
    let lower = 0;
...
function ricfMonthlyForParts(ricf, parts) {
    if (!Array.isArray(ricf))
        return 0;
 ...
 * à 25 %.
 */
function seniorityRate(hireDate) {
    if (!hireDate)
        return 0;
...
function computePrimeAnciennete(baseSalary, hireDate) {
...
function computePayroll(input, rates) {
    ...
    if (sursalaire > 0)
        lines.push({ type: 'GAIN', label: 'Sursalaire', amount: sursalaire, order: o++ });
    if (primeAnciennete > 0)
        lines.push({ type: 'GAIN', label: "Prime d'ancienneté", base: baseSalary, rate: input.senioriteRate, amount: primeAnciennete, order: o++ });
    if (taxablePrime > 0)
        lines.push({ type: 'GAIN', label: 'Primes imposables', amount: taxablePrime, order: o++ });
    if (overtime > 0)
    ...
    if (insurance > 0)
        lines.push({ type: 'RETENUE', label: 'Assurance maladie', base: insuranceBase, rate: rates.insuranceRate, amount: insurance, order: o++ });
 ...
 */
async function computePayslipTotals(payslip) {
    const db = (0, db_service_1.getDb)();
    const year = payslip.periodYear;
    const month = payslip.periodMonth; // 1-12
    ...
    const sum = (key) => ytd.reduce((s, p) => s + Number(p[key]), 0);
    // Heures de travail : jours ouvrés − congés payés − repos compensateur.
    const hoursPerDay = await (0, attendance_service_1.getWorkHoursPerDay)();
    ...
    const monthLeave = await (0, leave_service_1.approvedLeaveDaysInRange)(employeeId, monthStart, nextMonthStart);
    const ytdLeave = await (0, leave_service_1.approvedLeaveDaysInRange)(employeeId, yearStart, nextMonthStart);
    const monthWorkDays = Math.max(0, (0, leave_service_1.workingDays)(monthStart, monthLastDay) - monthLeave.congePaye - monthLeave.reposComp);
    const ytdWorkDays = Math.max(0, (0, leave_service_1.workingDays)(yearStart, monthLastDay) - ytdLeave.congePaye - ytdLeave.reposComp);
...
async function loadPayslipCompany() {
    const map = await (0, settings_service_1.getSettings)([
        settings_service_1.SettingsKeys.companyName, settings_service_1.SettingsKeys.companyRegistre, settings_service_1.SettingsKeys.companyContribuable,
        settings_service_1.SettingsKeys.companyAddress, settings_service_1.SettingsKeys.companyPhoneFixed,
    ]);
    return {
...
function payslipLayoutCss(layout, accent) {
    if (layout === 'MODELE_2') {
...
function renderPayslipHtml(payslip, employee, company, template, logoDataUri, counters, totals) {
    const esc = (v) => String(v ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const fmt = (n) => `${new Intl.NumberFormat('fr-FR').format(Math.round(Number(n ?? 0)))}`;
    const fmtDate = (d) => (d ? new Date(d).toLocaleDateString('fr-FR') : '—');
    /** Ancienneté en années/mois calculée à la fin de la période de paie. */
    const seniorityText = (hire) => {
        if (!hire)
            return '—';
    ...
    const paymentText = payslip.paidAt
        ...
        : '';
    const lines = (payslip.lines ?? []).slice().sort((a, b) => a.order - b.order);
    ...
    const fmtDays = (n) => new Intl.NumberFormat('fr-FR', { minimumFractionDigits: 0, maximumFractionDigits: 1 }).format(Number(n ?? 0));
    const dayCell = (v) => `<td class="num">${v == null ? '—' : fmtDays(v)}</td>`;
    const counterCells = (c) => `${dayCell(c.acquired)}${dayCell(c.takenYear)}${dayCell(c.remaining)}`;
