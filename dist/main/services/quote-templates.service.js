...
exports.seedDefaultQuoteTemplate = seedDefaultQuoteTemplate;
...
function buildFooter(color, text) {
 ...
 */
async function seedDefaultQuoteTemplate() {
    ...
    if (legacy && !classique) {
        ...
        logger_1.default.info('Quote template migrated: « Devis (par défaut) » → « Devis — Classique »');
    }
    ...
    for (const t of TEMPLATES) {
        ...
        logger_1.default.info(`Quote template created: ${t.name}`);
    }
    ...
    if (!anyDefault) {
        ...
        if (fallback)
            await db.quoteTemplate.update({ where: { id: fallback.id }, data: { isDefault: true } });
    }
}
