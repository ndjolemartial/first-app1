...
*Mise à jour 19 août 2026 (Formations LBC/FT/FP — pièces jointes partagées entre tous les participants, dès la création) — le formulaire **« Nouvelle formation »** (`AmlTrainingListPage.tsx`) permet désormais de sélectionner des **pièces jointes avant même l'enregistrement** (la session n'existe pas encore côté base — état local `pendingFiles`, ajout/retrait libre, même principe que le formulaire « Nouvelle innovation IT » du Module 16). À l'enregistrement, une fois les N lignes `AmlTraining` créées (une par participant, registre plat inchangé), ces pièces sont **importées séquentiellement sur chacune** via `documents:import` (`amlTrainingId` = id de chaque ligne) — une pièce jointe est donc **dupliquée** (fichier physique + ligne `Document` propres) sur la fiche de **chaque participant sélectionné**, et non plus visible du seul premier. Corrige au passage une lacune de la fonctionnalité déjà en place (bouton trombone par ligne, upload sur une seule ligne à la fois, sans aucune visibilité des pièces déjà jointes) : `aml:training:list` inclut désormais `_count.documents` (filtré `deletedAt: null`, même motif que `_count.attachments` du Module 16) — indicateur trombone + compteur par ligne du tableau, cliquable pour ouvrir une fenêtre listant les pièces jointes de cette ligne (nom, taille, bouton « Ouvrir » via `documents:open`), alimentée à la demande par `aml:training:getById` (déjà existant, déjà soumis aux mêmes règles de périmètre — un rôle restreint, cf. mise à jour du 11 août 2026, ne peut ouvrir que les pièces de ses propres formations). Aucune migration Prisma (réutilise `Document.amlTrainingId` existant) ; aucun changement de permissions (upload toujours réservé `AML_ROLES`, lecture ouverte aux mêmes rôles que `aml:training:list`/`getById`).*

*Mise à jour 19 août 2026 (correctif — pièces jointes sélectionnées à la création d'une Formation LBC/FT invisibles à l'écran) — deux bugs distincts empêchaient les pièces jointes ajoutées via le sélecteur de fichiers de « Nouvelle formation » (`AmlTrainingListPage.tsx`) d'apparaître dans la liste avant enregistrement :* ***(1)*** *l'input de fichier caché portait `className="hidden"` (`display: none`) — dans ce build Electron, la boîte de dialogue native de sélection de fichiers ne notifie jamais un `<input type="file">` qui a `display: none` au moment de la résolution (le `change` ne se déclenche tout simplement pas), contrairement à un élément simplement rendu hors champ visuel. Remplacé par un style « visuellement masqué mais toujours rendu » (`position: absolute`, 1×1 px, `clip`, `overflow: hidden` — motif `sr-only` standard), constante partagée `HIDDEN_FILE_INPUT_STYLE`, appliquée aux deux inputs cachés du fichier (sélection avant création + téléversement par ligne).* ***(2)*** *une fois le `change` corrigé, un second bug plus insidieux subsistait : `addPendingFiles` appelait `Array.from(list)` **à l'intérieur** du callback (différé) de `setPendingFiles((prev) => …)`, alors que `list` est une référence **live** vers le `FileList` de l'input — or `onChange` réinitialise `e.target.value = ''` juste après l'appel, ce qui **vide aussi ce FileList** ; si React exécute le callback après cette réinitialisation (constaté empiriquement : fonctionne une fois, puis échoue silencieusement dès le retrait/réajout suivant), `Array.from(list)` retombe sur un tableau vide. Corrigé en matérialisant `Array.from(list)` **immédiatement**, avant tout retour au code appelant, plutôt que dans le callback différé. Diagnostiqué par instrumentation temporaire (toast puis `console.log`) confirmant successivement que le `change` natif se déclenchait bien (une fois le CSS corrigé) mais que le tableau reconstruit à l'intérieur du callback `setState` était vide.

**Même correctif (2) appliqué par précaution aux 4 autres sélecteurs de pièces jointes du dépôt partageant l'exact même anti-motif** (`Array.from(list)` différé à l'intérieur d'un callback `setState`, avec réinitialisation de `input.value` juste après l'appel côté `onChange`) — repérés par recherche du motif dans le code, corrigés de façon identique (capture immédiate avant `setState`) : `OperationFormPage.tsx` (Trésorerie — pièces jointes d'une opération), `SendMessagePage.tsx` (Communication — pièces jointes d'un message), `PublicationModal.tsx` (Réseaux Sociaux — pièces jointes d'une publication), `ActivityFormPage.tsx` (CRM — pièces jointes d'une activité). Le correctif (1) (CSS `display:none` → visuellement masqué) n'a en revanche **pas** été généralisé aux autres inputs de fichiers cachés du dépôt (`PhaseAttachments.tsx`, `EmployeeDetailPage.tsx` « Contrats signés », `LeaveRequestsPage.tsx`, `AmlProfileDetailPage.tsx`) faute de confirmation empirique qu'ils sont concernés — ces 4 fichiers capturaient déjà correctement `Array.from()` de façon immédiate (pas de bug (2)), et rien n'indique que le `change` ne s'y déclenche pas ; si un symptôme identique (dialogue de fichier qui s'ouvre mais sélection sans aucun effet visible) devait y être observé, appliquer le même remplacement `className="hidden"` → `HIDDEN_FILE_INPUT_STYLE`.*

*Mise à jour 19 août 2026 (Moteur de devis de construction — renommage du lot « Clôture » et de l'ouvrage « Portail ») — sur les devis de construction (tous types de projet, pas seulement « Clôture seule » : `LOT19` est un lot partagé, sans variante par portée), le lot **« Clôture & portail »** est renommé **« Clôture »** (en-tête de groupe + ligne « Sous-total » sur les 5 documents PDF de l'estimation et sur le devis commercial converti, `ConstructionLot.label`, colonne `lotLabel` snapshotée sur chaque `ConstructionEstimateLine` à la génération — sans effet sur les estimations déjà générées, qui restent figées). L'ouvrage **« Portail métallique coulissant »** (`CLOT.PORTAIL`, LOT19) est renommé **« Portail »** — cet ouvrage apparaissait déjà comme une ligne de tableau à part entière, avec prix unitaire/quantité/montant comme tout autre ouvrage, dès que `gateCount ≥ 1` (`applicabilityRule: { field: 'gateCount', gt: 0 }`, formule `QTE_PORTAILS`) : seul le libellé change, le comportement conditionnel existait déjà. Les deux renommages sont de simples changements de données (seed idempotent `scripts/seed-construction.mjs`, upsert par `code` incluant `label`/`designation`), sans migration Prisma — appliqués à la base existante en ré-exécutant le script (`node scripts/seed-construction.mjs`). Par cohérence, la **catégorie** « Clôture & portail » du catalogue documentaire des formules (*Paramètres → « Formules de calcul »*, `constructionFormulasCatalog.ts`, purement documentaire/lecture seule) est renommée « Clôture » également (les 4 entrées `QTE_CLOTURE_ML`/`QTE_CLOTURE_SURFACE`/`QTE_CLOTURE_POTEAUX`/`QTE_PORTAILS`) — code TypeScript, sans donnée en base, effectif immédiatement au prochain rechargement.*

*Mise à jour 19 août 2026 (Moteur de devis de construction — ligne « Portail » toujours en dernière position du lot « Clôture ») — l'ordre des lignes d'un devis au sein d'un même lot suit `ConstructionWorkItem.sortOrder` (`orderBy: [{ lot: { numero: 'asc' } }, { sortOrder: 'asc' }]` dans `construction-engine.service.ts`, préservé tel quel jusqu'aux lignes d'estimation générées) — tous les 10 ouvrages actifs du lot « Clôture » (LOT19) portaient jusqu'ici le même `sortOrder` par défaut (0), l'ordre final dépendant alors de l'ordre de retour non garanti de la requête SQL à égalité. `CLOT.PORTAIL` reçoit désormais explicitement le `sortOrder` le plus élevé du lot (8, contre 0 à 7 pour fouille/fondation/ferraillage/chaînages/montage du mur/poteaux/crépissage) — la ligne « Portail » apparaît donc systématiquement en dernière position du groupe « Clôture », qu'elle qu'en soit la portée du devis (complet ou « Clôture seule »), dès que `gateCount ≥ 1`. Changement de données uniquement (seed idempotent, upsert par `code` incluant désormais `sortOrder`), sans migration Prisma.*
